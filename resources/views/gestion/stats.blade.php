...
@if (!(in_array($pausa->Agente,$operadoresTrabajaron))) 
    <?php
...
@if (!(in_array($saliente->Agente,$operadoresTrabajaron))) 
    <?php
...
@endif 
@endforeach
@foreach ($holdtime as $hold) 
@if (!(in_array($hold->Agente,$operadoresTrabajaron))) 
    <?php
...
@if (!(in_array($entrante->Agente,$operadoresTrabajaron))) 
    <?php
...
@foreach ($operadoresTrabajaron as $operador)
@foreach ($empleados as $empleado)
    @if($operador==$empleado->nombre)
...
@for($i = 0; $i < count($operadoresObj); $i++)
<!--busco coincidencias en pausas y agrego al operador-->
@foreach ($pausas as $pausa)
@if($operadoresObj[$i]->nombre==$pausa->Agente)
<?php 
...
@foreach ($entrantes as $entrante)
@if($operadoresObj[$i]->nombre==$entrante->Agente)
<?php 
...
@foreach ($salientes as $saliente)
@if($operadoresObj[$i]->nombre==$saliente->Agente)
<?php 
...
@foreach ($holdtime as $hold)
@if($operadoresObj[$i]->nombre==$hold->Agente)
<?php 
...
<table class="table table-sm table-bordered " >
    ...
    <tbody>
        <?php $sup; ?>
        <?php $cat;?>
@foreach ($operadoresObj as $operador )  
        
          <tr>
           <td>{{$operador->nombre}}</td>
           @foreach ($empleados as $empleado)
               @if ($operador->nombre==$empleado->nombre)
               <td>{{$sup=$empleado->supervisor}}</td>
               <td>{{$cat=$empleado->categoria}}</td>            
               @endif
           @endforeach
           <?php 
            if (!isset($operador->sessionTime)) {
                $operador->sessionTime = '00:00:00';
               echo' <td>' ?>{{$operador->sessionTime}} <?php '</td>';
            }else {
                echo '<td>'?>{{$operador->sessionTime}}<?php '</td>';
            }
           ?>
           <td>{{$operador->cantEntrantesAtendidas+$operador->cantSalientesAtendidas}}</td>
           <td>{{$operador->cantEntrantesAtendidas}}</td>
           <td>{{$operador->cantSalientesAtendidas}}</td>
           <td>{{$operador->acwCantidad}}</td>
           <td>{{$operador->acwTiempo}}</td>
           ...
           <td>{{$operador->holdTimeTotal}}</td>
           <td>{{$operador->almuerzoTiempo}}</td>
           <td>{{$operador->backofficeTiempo}}</td>
           <td>{{$operador->reunionTiempo}}</td>
