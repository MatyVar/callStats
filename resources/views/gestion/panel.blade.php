...
<div class="row">
    ...
    <div class="col">
        ...
        <select name="supervisores" class="form-control" id="supervisores">
            ...
            <option value="{{$supervisor->nombre}}" class="form-control">{{$supervisor->nombre}}</option>
            @endforeach
        </select>
      <div class="card mt-2">
        <p class="card-header">Gestiones:</p>
        <div class="card-body">
            <a class="btn btn-info d-block mt-2" data-toggle="modal" data-target="#exampleModalLong">Coaching Express  <img src="{{asset('img/summarize_white_24dp.svg')}}" alt=""></a>
        <!-- Modal para consulta de notas por cantidad -->
        <a type="button" class="btn btn-success d-block mt-2" data-toggle="modal" data-target="#exampleModalLong2">Notas por cantidad  <img src="{{asset('img/confirmation_number_white_24dp.svg')}}" alt=""></a>
        <a type="button" class="btn btn-danger d-block mt-2" href="{{route('gestion.reportes')}}">Reporting  <img src="{{asset('img/confirmation_number_white_24dp.svg')}}" alt=""></a>
        </div>
      </div>
    </div>
<a href="{{route('gestion.vistaPorHora')}}" style="text-decoration:none;">
    <div class="col">
        ...
        <table class="table table-sm table-bordered alert alert-warning">
            ...
            <tbody>
                <tr>
                    ...
                    @foreach ($de14a22 as $item )
                        <td class="alert alert-success">{{$item}}</td>
                    @endforeach
                  </tr> 
              <tr>
                <tr>
                    <th scope="row">10:00 a 18:00</th>
                    @foreach ($de10a18 as $item )
                    <td class="alert alert-danger">{{$item}}</td>
                @endforeach
                  </tr>
                  <tr>
                    <th scope="row">09:00 a 16:00</th>
                    @foreach ($de9a16 as $item )
                    <td class="alert alert-warning">{{$item}}</td>
                @endforeach
                  </tr>
                  <tr>
                    <th scope="row">09:00 a 13:00</th>
                    @foreach ($de9a13 as $item )
                    <td>{{$item}}</td>
                @endforeach
                  </tr>
                ...
                @endforeach
                  ...
                  <tr>
                    ...
                    @foreach ($de8a16 as $item )
                        <td class="alert alert-dark">{{$item}}</td>
                    @endforeach
                  </tr>
                  <tr>
                    <th scope="row">07:00 a 15:00</th>
                    @foreach ($de7a15 as $item )
                        <td>{{$item}}</td>
                    @endforeach
                  </tr>
                <th scope="row">07:00 a 14:00</th>
                @foreach ($de7a14 as $item )
                <td>{{$item}}</td>
            @endforeach
              </tr>
...
<div class="col-lg-10">
    ...
    <table class="table  table-hover table-striped" id="tableOperadores">
        ...
        <tbody>
            ...
            <tr class="{{$operador->supervisor}}" id="sup">
                <td><img style="opacity: 0.7; margin-bottom: 5px; width: 30px;margin-bottom: 4px;"  src="{{asset('img/support_agent_black_24dp.svg')}}" alt=""><b style="margin-left: 5px">{{$operador->nombre}}</b></td>
                <td>{{$operador->categoria}}</td>
                <td>{{$operador->supervisor}}</td>
                @if (($operador->diasAFavor)==0)
               <td><span></span></td>
                @else
                @if (($operador->diasAFavor)> 0 && ($operador->diasAFavor)< 2 )
                <td><span class="badge badge-warning text-white">{{$operador->diasAFavor}}</span></td>
                @else
                <td><span class="badge badge-danger">{{$operador->diasAFavor}}</span></td>
                @endif
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item dropdown ">
                            <a id="navbarDropdown" class=" dropdown-toggle btn" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                                <img src="{{asset('/img/timeline_black_24dp.svg')}}" alt=""> Vista
                            </a>
                            ...
                            <div class="dropdown-menu  " style="background-color: #f6f6f6 ;" aria-labelledby="navbarDropdown">
                                ...
                                <a href="{{route('gestion.fichero', $operador->legajo)}}" class="dropdown-item">Fichadas</a>
                                <a href="{{route('gestion.edit', ['operador'=>$operador->id])}}" class="dropdown-item">Editar</a>
                                <a href="{{route('gestion.escuchas',$operador->nombre)}}" class="dropdown-item">Ver registros de Coaching Express</a>
                                <a href="{{route('gestion.notas', ['operador'=>$operador->id])}}" class="dropdown-item ">Gestionar notas</a>
                                <a href="{{route('gestion.acciones', ['operador'=>$operador->id])}}" class="dropdown-item ">Registrar compensacion de dia completo</a>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <form class="form-group" method="POST" action="{{route('gestion.recolectarNotas')}}">
                        ...
                        <select required name="tipo" id="" class="form-control">
                            @foreach ($nombresNotas as $nombre)
                            <option value="{{$nombre->nombreNota}}">{{$nombre->nombreNota}}</option>
                            @endforeach
